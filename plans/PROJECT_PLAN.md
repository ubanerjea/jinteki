...
8. **Decklist advanced search & quick views** — `/decklists` rebuilt as a quick-views landing page (Recent/Recently updated/Posted this week/Favorited by jinteki users), `/decklists/advanced` structured search mirroring the card side, old simple search removed. Deliberately does **not** build NRDB's Popular/Hall of Fame/Hot Topics/Tournaments tabs or deck-wide legality — see `plans/PHASE_8_PLAN.md`'s Divergence section and "Explicitly deferred" list. Shipped; addendum added a Format (card-pool-membership) filter.
