...
1. ~~**GitHub OAuth App creation**~~ — **Resolved 2026-07-28.** The repo owner created a real GitHub OAuth App and populated `AUTH_GITHUB_ID`/`AUTH_GITHUB_SECRET` in `.env` (verified: 20-char client ID, 40-char secret, both non-empty).
2. ~~**First admin promotion**~~ — **Resolved 2026-07-28.** The owner signed in for real via GitHub; a `User` row was created (`unmeel@gmail.com`, linked `Account` with GitHub `providerAccountId`, valid `Session` row), confirmed directly via `psql`. Promoted to `ADMIN` via the documented one-off `UPDATE "User" SET role = 'ADMIN' WHERE email = ...` (`src/lib/require-admin.ts`'s comment), confirmed via `psql` (`role` column now `ADMIN`).
3. ~~**Full GitHub OAuth login flow**~~ — **Resolved 2026-07-28**, verified end-to-end for real (not just route wiring): real sign-in created real `User`/`Account`/`Session` rows (checked via `psql`); the real session token from that `Session` row was then used directly as a cookie (`authjs.session-token=<token>`) in `curl` requests against the running `pnpm dev` server, which returned the actual authenticated `/admin/sync` page (the real sync table, not "Access denied") and successfully triggered a live sync via `POST /api/admin/sync/factions-packs` (`200`, real `SyncRun` row created, confirmed via `psql`). No browser automation was available for this verification, so the login page itself was completed manually by the repo owner, but everything after that — session validity, admin gating, and the full authenticated write path — was independently verified with real HTTP requests and real database checks.
